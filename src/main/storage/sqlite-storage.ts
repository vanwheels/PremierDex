...
export function createSqliteStorage(dbPath: string): StorageAdapter {
  ...
  const listSpeciesStmt = db.prepare(
    'SELECT id, name, generation, collapsed_display_form_id, is_final_evolution_stage, evolves_from_species_id FROM species ORDER BY id'
  )
  const getSpeciesStmt = db.prepare(
    'SELECT id, name, generation, collapsed_display_form_id, is_final_evolution_stage, evolves_from_species_id FROM species WHERE id = ?'
  )
