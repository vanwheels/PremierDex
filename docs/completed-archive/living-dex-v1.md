# Completed — Living Dex v1 milestone

Archived from `COMPLETED.md` at the Living Dex v1 milestone boundary. See
`docs/postmortems/living-dex-v1.md` for the milestone writeup. Newest first, same order
`COMPLETED.md` kept them in.
...
to the display name in `DexRow.tsx` when false, styled in `global.css`. See commit
`0aa6a29` — this leg's own notes originally read "not a git working directory, no commit
hash," which was wrong; the repo existed the whole time. Full change is `types.ts`,
`buildDexSections.ts`, `DexRow.tsx`, `global.css`, plus a new `buildDexSections.test.ts`
case asserting the field passes through instead of filtering. All 47 existing tests +
